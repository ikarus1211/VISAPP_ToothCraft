...
from torch.optim.lr_scheduler import LambdaLR, StepLR, CosineAnnealingLR, CosineAnnealingWarmRestarts
from utils.scheduler import DecayingCosineAnnealingWarmRestarts, CosineDecayWithWarmup
...
def initialize_scheduler(optimizer, config, last_step=-1):
  if config.scheduler == 'StepLR':
    return StepLR(
      ...
      return CosineDecayWithWarmup(optimizer, warmup_steps=config.warmup ,T_max=config.step_size, eta_min=float(config.eta_min))
  elif config.scheduler == 'DecayCosine':
      return DecayingCosineAnnealingWarmRestarts(optimizer, decay=config.decay, T_mult=2, T_0=config.step_size, eta_min=float(config.eta_min))
